...
** In-text linking and highlighting to a specific PDF region
Everything revolves around one customizable variable ~org-noter-store-link-markup-annotation~.

It accepts *three* values:
| Value  | PDF Modified? | Region Stored in Link? | On Open Behaviour          |
|--------+---------------+------------------------+----------------------------|
| =t=      | YES           | YES                    | Flashes the saved region   |
| ='flash= | NO            | YES                    | Flashes the saved region   |
| =nil=    | NO            | NO                     | Goes to page/location only |

Choose your preferred default by adding ONE of these to your config:
#+BEGIN_SRC emacs-lisp
;; Option 1: Permanently annotate the PDF
(setq org-noter-store-link-markup-annotation t)

;; Option 2: Store region coords for flashing, but DON'T touch the PDF
(setq org-noter-store-link-markup-annotation 'flash)

;; Option 3: No highlighting and no flashing
(setq org-noter-store-link-markup-annotation nil)
#+END_SRC

*** How to use
- Highlight a piece of text (you can also make a rectangular
  region by holding the ~META~ key and dragging)
- Press ~C-c l~
- Come back to your notes file and press ~C-c C-l~ to paste it

*** Demo videos
- Here is a [[../Demo/Media/annotate.mp4][demo video]] of =t=.
- And here is [[../Demo/Media/flash.mp4][one]] of ='flash=.

*** On-fly behaviour change
You don't have to change your config to switch modes temporarily.
Use a *prefix argument* (ie ~C-u C-c l~) when storing the link:

| Current Setting | Effective Behaviour with ~C-u~ Prefix |
|-----------------+---------------------|
| =t= (annotate)    | =nil= (nothing)       |
| ='flash=          | =nil= (nothing)       |
| =nil= (nothing)   | =t= (annotate)        |

This is handy when you only occasionally want permanent annotation (or
vice versa).
